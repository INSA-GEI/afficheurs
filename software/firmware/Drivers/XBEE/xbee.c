...
char tx_frame_buffer[18+20];
char rx_frame_buffer[18+0x80];
...
#define XBEE_TIMEOUT_FOR_STATUS_FRAME		4000
...
int XBEE_EnterCommandMode()
{
	...
	status=XBEE_LL_ReceiveData(buffer, 3, 2000);
	if (status!=XBEE_LL_OK)
		return XBEE_CONFIG_ERROR;
...
int XBEE_SetATCommand(char* atcmd)
{
	...
	if (XBEE_LL_ReceiveData(buffer, 3, 500) != XBEE_LL_OK)
		return XBEE_AT_CMD_ERROR;
...
int XBEE_GetATValue(char* atcmd, char* value)
{
	...
	while (tmp!='\r')
	{
		if (XBEE_LL_ReceiveData(&tmp, 1, 500) != XBEE_LL_OK)
			return XBEE_AT_CMD_ERROR;

		if (index >=20) return XBEE_AT_CMD_ERROR;
...
void XBEE_EncodeTransmissionFrame_Zigbee(char* frame, char* data, uint16_t destination, uint8_t frame_id)
{
	int i;
	int data_length = strlen(data);
	uint8_t checksum;
	uint64_t dest_long=0xFFFFFFFFFFFFFFFF;
	...
	for (i=5; i<5+8; i++) {
		frame[i] = (char)(dest_long >>(64-8));
		dest_long = dest_long <<8;
	}

	frame[13] = (uint8_t)(destination>>8);
	frame[14] = (uint8_t)(destination & 0xFF);
	frame[15] = 0;
	frame[16] = 0x0;
...
int XBEE_DecodeFrame(char* frame, XBEE_GENERIC_FRAME** decoded_frame)
{
	...
	case XBEE_RX_PACKET_TYPE:
		*decoded_frame = (XBEE_GENERIC_FRAME*)malloc(sizeof(XBEE_RX_PACKET_FRAME)+(frame_length-12)+1);
		...
		((XBEE_RX_PACKET_FRAME*)(*decoded_frame))->data[((XBEE_RX_PACKET_FRAME*)(*decoded_frame))->data_length]=0x0;
		break;
...
int XBEE_Init (void)
{
	...
	XBEE_LL_ConfigureUart(USART1, 9600);

	if (XBEE_ConfigureDevice() != XBEE_OK)
		return XBEE_CONFIG_ERROR;

	XBEE_LL_ConfigureUart(USART1, 115200);
...
int XBEE_SendData_Zigbee(uint16_t destination, uint8_t frame_id, char* data, uint8_t *status)
{
	...
	XBEE_EncodeTransmissionFrame_Zigbee(tx_frame_buffer, data, destination, frame_id);
	if (XBEE_SendFrame(tx_frame_buffer) != XBEE_OK)
		return XBEE_TX_ERROR;


	if (XBEE_GetFrame(rx_frame_buffer, XBEE_TIMEOUT_FOR_STATUS_FRAME) != XBEE_OK)
		return XBEE_RX_ERROR;
...
{
	int status;
	status = XBEE_LL_ReceiveData(rx_frame_buffer, -1, 5000);
